...
namespace ChallengeSandbox
{
    public class LeetcodeChallenges
    {
        ...
        {
            // this is a weird challenge since we are forced to make certain assumptions about these arrays based on the order they are passed into the method

            // handle edge cases
            if(nums2.Length == 0)
            {
                // do nothing, as first array is sorted and second array is empty
                return;
            }
            else if (!nums1.Any(n => n != 0))
            {
                // first array only contains zeroes, return the second array inserted into the first
                for(int i = 0; i < n; i++)
                {
                    nums1[i] = nums2[i];
                }
            ...
            // point to the current items being compared
            var nums1Pointer = 0;
            var nums2Pointer = 0;
            var mergedIndex = 0;

            // it will be simpler to track the state of the merged array independently from the values being compared
            // particularly since arrays cannot be dynamically resized in C#
            var mergedArray = new int[nums1.Length + nums2.Length];

            while(nums1Pointer < nums1.Length && nums2Pointer < nums2.Length && nums1Pointer < m && nums2Pointer < n)
            {
                if (nums1[nums1Pointer] < nums2[nums2Pointer])
                {
                    mergedArray[mergedIndex] = nums1[nums1Pointer];
                    nums1Pointer++;
                }
                else if (nums1[nums1Pointer] > nums2[nums2Pointer])
                {
                    mergedArray[mergedIndex] = nums2[nums2Pointer];
                    nums2Pointer++;
                }
                else if (nums1[nums1Pointer] == nums2[nums2Pointer])
                {
                    // arbitrarily pick nums1 if they are equal
                    mergedArray[mergedIndex] = nums1[nums1Pointer];
                    nums1Pointer++;
                }
            ...
            // based on which condition we just tripped, fill out the rest of the array
            if(nums1Pointer == nums1.Length || nums1Pointer == m)
            {
                // do nothing? maybe? it _shouldn't_ be possible for length of nums2 to exceed nums1
                for(int i = nums2Pointer; i < nums2.Length; i++)
                {
                    mergedArray[mergedIndex] = nums2[i];
                    mergedIndex++;
                }
            }
            
            if (nums2Pointer == nums2.Length || nums2Pointer == n) // done with nums2, but _maybe_ not nums1
            {
            ...
            // now that we have a merged array, push the values back into the original
